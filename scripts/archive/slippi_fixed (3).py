# v19-bulletproof-numpy: Fix constraint file creation order
import modal
...
def create_slippi_image():
    ...
    # CRITICAL: Create constraint file and pip.conf FIRST before setting environment
    image = image.run_commands([
        ...
        "cat /root/numpy-constraint.txt"
    ])

    # Now install NumPy with the constraint file in place
    image = image.run_commands([
    ...
    # Install Rust and maturin for peppi-py (with NumPy constraint already active)
    image = image.run_commands([
        "curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y",
        # Install maturin as both Python package AND Cargo binary with constraint active
        "python3 -m pip install maturin==1.2.3",
        "/root/.cargo/bin/cargo install maturin",
        # Verify maturin is available both ways
        "python3 -c 'import maturin; print(\"✅ maturin Python module available\")'",
    ...
    # Install peppi-py with NumPy constraint active and proper Rust environment
    image = image.run_commands([
        # Verify maturin is available before attempting peppi-py build
        "python3 -c 'import maturin; print(\"maturin ready for peppi-py build\")'",
        # Install peppi-py with no build isolation so it can access maturin
    ...
    image = image.run_commands([
        ...
        'python3 -c "import tensorflow as tf; print(f\'✅ TensorFlow test: {tf.constant([1,2,3]).numpy()}\')"'
    ]).env({
        "PIP_CONSTRAINT": "/root/numpy-constraint.txt"
    })
...
app = modal.App("slippi-ai-bulletproof-numpy-v19")
