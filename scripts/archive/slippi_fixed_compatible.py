...
def create_slippi_image():
    ...
    # ✅ Lock numpy before anything else, prevent upgrades - EXACT COPY FROM WORKING VERSION
    image = image.run_commands([
        "python3 -m pip install numpy==1.24.3 --no-deps"
    ])

    # ✅ Install peppi-py with correct backend and no isolation - EXACT COPY FROM WORKING VERSION
    image = image.run_commands([
    ...
    # ✅ Pin all scientific deps AFTER numpy is frozen - EXACT VERSIONS FROM WORKING SCRIPT
...
def comprehensive_test():
    ...
    # Run the original working test first
    try:
        print("🔬 Running original working test...")
        import peppi_py
        import tensorflow as tf
        import jax
        import sacred
        ...
        # Additional TensorFlow tests that failed before
        test_tensor = tf.constant([1.0, 2.0, 3.0])
        mean_result = tf.reduce_mean(test_tensor)
        print(f"✅ TensorFlow mean test: {mean_result.numpy()}")
        
        # Test TensorFlow-NumPy integration
        tf_array = tf.constant([1, 2, 3])
        np_array = tf_array.numpy()
        print(f"✅ TensorFlow-NumPy integration: {np_array}")
...
def main():
    ...
    if success:
        ...
        print("\n❌ Environment test failed - this shouldn't happen with proven config")
