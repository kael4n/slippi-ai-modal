...
def create_slippi_image():
    ...
    image = image.pip_install([
        ...
        "gymnasium==0.28.1",
        "gym==0.21=6.2",  # For backward compatibility
    ])
...
def _input(prompt):
    """Safe input wrapper for Modal or non-interactive environments."""
    try:
        return input(prompt)
    except EOFError:
        print("Input not available (non-interactive environment). Using defaults.")
        return ""
...
def main():
    ...
    while True:
        ...
        choice = _input("Enter choice (1-6): ").strip() or "6"

        if choice == "1":
            print("\n🔍 Running environment validation...")
            result = validate_environment.remote().get()
            print(f"Validation result: {json.dumps(result, indent=2)}")

        elif choice == "2":
            print("\n🧪 Running quick test...")
            quick_test.remote().get()

        elif choice == "3":
            print("\n⚙️ Processing replays...")
            result = process_replays.remote().get()
            print(f"Processing result: {json.dumps(result, indent=2)}")

        elif choice == "4":
            print("\n🚀 Starting training...")

            use_custom_config = _input("Use custom configuration? (y/n): ").strip().lower()
            config_overrides = None

            if use_custom_config == 'y':
                print("Enter configuration overrides (JSON format, or press Enter for defaults):")
                config_input = _input("").strip()
                if config_input:
                    try:
            ...
            result = train_slippi_ai.remote(config_overrides).get()
            print(f"Training result: {json.dumps(result, indent=2)}")

        elif choice == "5":
            print("\n🧹 Cleaning up models...")
            keep_count = _input("Number of checkpoints to keep per experiment (default 3): ").strip()
            keep_count = int(keep_count) if keep_count.isdigit() else 3

            result = cleanup_models.remote(keep_count).get()
