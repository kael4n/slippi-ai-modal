# Final v7: use `python3 -m pip` instead of `pip` to avoid not found errors
import modal
...
def create_slippi_image():
    ...
    image = image.run_commands([
        f". ~/.cargo/env && export PATH='/root/.cargo/bin:$PATH' && python3 -m pip install --no-build-isolation --verbose --timeout=300 '{PEPPI_PY_URL}'",
        'python3 -c "import peppi; print(\'peppi-py installed and importable\')"'
    ])
...
app = modal.App("slippi-ai-strict-clean-v7")
