...
image = modal.Image.debian_slim().apt_install(
    ...
    "pip install -e .",
    # --- DEFINITIVE FIX ---
    # Hardcode the data_dir path to bypass the application's complex config handling.
    # Note the use of `|` as a delimiter and escaped quotes to handle the file path.
    'sed -i "s|train_lib.train(config)|config.dataset.data_dir = \\"/data/games/Ga\\"; train_lib.train(config)|" scripts/train.py',
)
...
def train():
    ...
    # We still pass replays_path here, as the script may use it for other purposes,
    # but our injected code now guarantees data_dir is set correctly.
    command = [
        "python", "scripts/train.py",
        "with",
        f"replays_path={replays_path}",
    ]
    
    print(f"👟 Running training command: {' '.join(command)}\n")

    # The PYTHONPATH adjustment is still valuable to ensure all sub-imports work correctly.
    env = os.environ.copy()
